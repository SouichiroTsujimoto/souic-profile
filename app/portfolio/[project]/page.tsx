...
export default function ProjectPage({
	...
	useEffect(() => {
		...
		params
			.then(({ project }) => {
				...
				if (!selectedProject) {
					console.log(
						"ProjectPage: project not found, throwing error",
					);
					throw new Error(
						`プロジェクト "${projectName}" が見つかりません`,
					);
				}
	...
	// selectedProjectがnullの場合は何も表示しない（loading.tsxまたはerror.tsxが処理）
	if (!selectedProject) {
		return null;
	}
